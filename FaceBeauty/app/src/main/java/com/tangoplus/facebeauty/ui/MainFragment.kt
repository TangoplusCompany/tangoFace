...
//                Log.v("displayList", "${mvm.displayList.value}")
                ...
                if (mvm.isMeasureFinish) {
                    val latestServerSn = mvm.currentFaceDisplays.firstOrNull()?.tempServerSn ?: 0
                    lifecycleScope.launch(Dispatchers.IO) {
                        val statics =  fDao.getStatic(latestServerSn)
                        withContext(Dispatchers.Main) {
                            mvm.currentResult.value = mvm.convertToFaceResult(requireContext(), statics)
...
//                Log.v("displayList", "${mvm.displayList.value}")
                ...
                cancelShimmer()
                val countText = "총 기록 건수: ${mvm.currentFaceDisplays.size}건"
                bd.tvMMeasureCount.text = countText
                setAdapter(mvm.currentFaceDisplays, 2)

                mvm.dataLoadComplete.value = false

                if (mvm.currentFaceDisplays.size == 0) {
...
//            Log.v("어댑터데이터", "${mvm.currentFaceResults}")
    ...
    private fun setComparisonClick() {
        bd.btnMComparision.setOnSingleClickListener {
            ...
            if (mvm.currentFaceDisplays.size < 2) {
                val balloon = Balloon.Builder(requireContext())
                    .setWidth(BalloonSizeSpec.WRAP)
                    .setHeight(BalloonSizeSpec.WRAP)
                    .setText("측정 기록이 최소 2개 이상 있어야 합니다!\n우측 하단의 버튼을 눌러 측정 해보세요")
            ...
            if (mvm.tempComparisonItems.value?.size == 2 && bd.btnMComparision.text == "결과 보기") {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    val leftStatics = sortedTempItems?.get(0)?.tempServerSn?.let { it1 -> fDao.getStatic(it1) }
                    val rightStatics = sortedTempItems?.get(1)?.tempServerSn?.let { it1 -> fDao.getStatic(it1) }

                    withContext(Dispatchers.Main) {
                        val leftResult = leftStatics?.let { it1 -> mvm.convertToFaceResult(requireContext(), it1) }
                        val rightResult = rightStatics?.let { it1 -> mvm.convertToFaceResult(requireContext(), it1) }
...
//                bd.btnMComparision.isEnabled = false
        ...
        bd.ibtnMNavi.setOnSingleClickListener {
            if (bd.clMNavi.isVisible) {
                ...
                animateExpand(requireContext(), bd.clMNavi,   false) {
                    ...
                    bd.etMSearch.setText("")
                    setAdapter(mvm.currentFaceDisplays,4)
                }
            } else {
                // 보이기
                ...
                if (afterText.isNotEmpty()) {
                    when {
                        ...
                        afterText.all { it.isDigit() } -> {
                            ...
                            if (number != null) {
                                filteredResult = mvm.currentFaceDisplays.filter {
    ...
    override fun onMeasureClick(tempServerSn: Int) {
        lifecycleScope.launch(Dispatchers.IO) {
            val statics =  fDao.getStatic(tempServerSn)
